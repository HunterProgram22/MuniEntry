# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'package\views\ui\MainWindow.ui'
#
# Created by: PyQt5 UI code generator 5.15.6
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(979, 700)
        MainWindow.setMinimumSize(QtCore.QSize(860, 700))
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setWeight(75)
        MainWindow.setFont(font)
        MainWindow.setStyleSheet("font: 75 11pt \"Palatino Linotype\";\n"
"color: rgb(0, 0, 0);\n"
"selection-color: rgb(0, 0, 0);\n"
"font-weight: bold;")
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout_5 = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout_5.setObjectName("gridLayout_5")
        self.tabWidget = QtWidgets.QTabWidget(self.centralwidget)
        self.tabWidget.setObjectName("tabWidget")
        self.crimTab = QtWidgets.QWidget()
        self.crimTab.setObjectName("crimTab")
        self.gridLayout_2 = QtWidgets.QGridLayout(self.crimTab)
        self.gridLayout_2.setHorizontalSpacing(18)
        self.gridLayout_2.setVerticalSpacing(20)
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.hemmeter_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.hemmeter_radioButton.setObjectName("hemmeter_radioButton")
        self.buttonGroup = QtWidgets.QButtonGroup(MainWindow)
        self.buttonGroup.setObjectName("buttonGroup")
        self.buttonGroup.addButton(self.hemmeter_radioButton)
        self.gridLayout_2.addWidget(self.hemmeter_radioButton, 0, 1, 1, 1)
        self.final_pretrial_cases_box = QtWidgets.QComboBox(self.crimTab)
        self.final_pretrial_cases_box.setEnabled(False)
        self.final_pretrial_cases_box.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.final_pretrial_cases_box.setEditable(False)
        self.final_pretrial_cases_box.setObjectName("final_pretrial_cases_box")
        self.gridLayout_2.addWidget(self.final_pretrial_cases_box, 5, 2, 1, 1)
        self.rohrer_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.rohrer_radioButton.setObjectName("rohrer_radioButton")
        self.buttonGroup.addButton(self.rohrer_radioButton)
        self.gridLayout_2.addWidget(self.rohrer_radioButton, 0, 2, 1, 1)
        self.pleas_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.pleas_radioButton.setObjectName("pleas_radioButton")
        self.buttonGroup_2 = QtWidgets.QButtonGroup(MainWindow)
        self.buttonGroup_2.setObjectName("buttonGroup_2")
        self.buttonGroup_2.addButton(self.pleas_radioButton)
        self.gridLayout_2.addWidget(self.pleas_radioButton, 6, 0, 1, 1)
        self.trials_to_court_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.trials_to_court_radioButton.setObjectName("trials_to_court_radioButton")
        self.buttonGroup_2.addButton(self.trials_to_court_radioButton)
        self.gridLayout_2.addWidget(self.trials_to_court_radioButton, 6, 1, 1, 1)
        self.kudela_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.kudela_radioButton.setObjectName("kudela_radioButton")
        self.buttonGroup.addButton(self.kudela_radioButton)
        self.gridLayout_2.addWidget(self.kudela_radioButton, 1, 2, 1, 1)
        self.pleas_cases_box = QtWidgets.QComboBox(self.crimTab)
        self.pleas_cases_box.setEnabled(False)
        self.pleas_cases_box.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.pleas_cases_box.setObjectName("pleas_cases_box")
        self.gridLayout_2.addWidget(self.pleas_cases_box, 7, 0, 1, 1)
        self.pelanda_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.pelanda_radioButton.setObjectName("pelanda_radioButton")
        self.buttonGroup.addButton(self.pelanda_radioButton)
        self.gridLayout_2.addWidget(self.pelanda_radioButton, 1, 1, 1, 1)
        self.label_3 = QtWidgets.QLabel(self.crimTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_3.setFont(font)
        self.label_3.setAlignment(QtCore.Qt.AlignCenter)
        self.label_3.setObjectName("label_3")
        self.gridLayout_2.addWidget(self.label_3, 3, 0, 1, 3)
        self.trials_to_court_cases_box = QtWidgets.QComboBox(self.crimTab)
        self.trials_to_court_cases_box.setEnabled(False)
        self.trials_to_court_cases_box.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.trials_to_court_cases_box.setObjectName("trials_to_court_cases_box")
        self.gridLayout_2.addWidget(self.trials_to_court_cases_box, 7, 1, 1, 1)
        self.slated_cases_box = QtWidgets.QComboBox(self.crimTab)
        self.slated_cases_box.setEnabled(False)
        self.slated_cases_box.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.slated_cases_box.setEditable(False)
        self.slated_cases_box.setObjectName("slated_cases_box")
        self.gridLayout_2.addWidget(self.slated_cases_box, 5, 1, 1, 1)
        self.arraignments_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.arraignments_radioButton.setObjectName("arraignments_radioButton")
        self.buttonGroup_2.addButton(self.arraignments_radioButton)
        self.gridLayout_2.addWidget(self.arraignments_radioButton, 4, 0, 1, 1)
        self.arraignments_cases_box = QtWidgets.QComboBox(self.crimTab)
        self.arraignments_cases_box.setEnabled(False)
        self.arraignments_cases_box.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.arraignments_cases_box.setEditable(False)
        self.arraignments_cases_box.setObjectName("arraignments_cases_box")
        self.gridLayout_2.addWidget(self.arraignments_cases_box, 5, 0, 1, 1)
        self.line = QtWidgets.QFrame(self.crimTab)
        self.line.setFrameShadow(QtWidgets.QFrame.Raised)
        self.line.setLineWidth(2)
        self.line.setFrameShape(QtWidgets.QFrame.HLine)
        self.line.setObjectName("line")
        self.gridLayout_2.addWidget(self.line, 2, 0, 1, 3)
        self.NotGuiltyBondButton = QtWidgets.QPushButton(self.crimTab)
        self.NotGuiltyBondButton.setMinimumSize(QtCore.QSize(120, 30))
        self.NotGuiltyBondButton.setStyleSheet("background-color: rgb(255, 255, 127);")
        self.NotGuiltyBondButton.setObjectName("NotGuiltyBondButton")
        self.gridLayout_2.addWidget(self.NotGuiltyBondButton, 11, 2, 1, 1)
        self.slated_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.slated_radioButton.setObjectName("slated_radioButton")
        self.buttonGroup_2.addButton(self.slated_radioButton)
        self.gridLayout_2.addWidget(self.slated_radioButton, 4, 1, 1, 1)
        self.label = QtWidgets.QLabel(self.crimTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setWeight(75)
        self.label.setFont(font)
        self.label.setObjectName("label")
        self.gridLayout_2.addWidget(self.label, 0, 0, 1, 1)
        self.FineOnlyPleaButton = QtWidgets.QPushButton(self.crimTab)
        self.FineOnlyPleaButton.setMinimumSize(QtCore.QSize(120, 30))
        self.FineOnlyPleaButton.setStyleSheet("background-color: rgb(85, 170, 127);")
        self.FineOnlyPleaButton.setObjectName("FineOnlyPleaButton")
        self.gridLayout_2.addWidget(self.FineOnlyPleaButton, 11, 0, 1, 1)
        self.JailCCPleaButton = QtWidgets.QPushButton(self.crimTab)
        self.JailCCPleaButton.setMinimumSize(QtCore.QSize(120, 30))
        self.JailCCPleaButton.setStyleSheet("background-color: rgb(85, 170, 127);")
        self.JailCCPleaButton.setObjectName("JailCCPleaButton")
        self.gridLayout_2.addWidget(self.JailCCPleaButton, 12, 0, 1, 1)
        self.bunner_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.bunner_radioButton.setObjectName("bunner_radioButton")
        self.buttonGroup.addButton(self.bunner_radioButton)
        self.gridLayout_2.addWidget(self.bunner_radioButton, 1, 0, 1, 1)
        self.final_pretrial_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.final_pretrial_radioButton.setObjectName("final_pretrial_radioButton")
        self.buttonGroup_2.addButton(self.final_pretrial_radioButton)
        self.gridLayout_2.addWidget(self.final_pretrial_radioButton, 4, 2, 1, 1)
        self.ProbationViolationBondButton = QtWidgets.QPushButton(self.crimTab)
        self.ProbationViolationBondButton.setMinimumSize(QtCore.QSize(120, 30))
        self.ProbationViolationBondButton.setStyleSheet("background-color: rgb(255, 255, 127);")
        self.ProbationViolationBondButton.setObjectName("ProbationViolationBondButton")
        self.gridLayout_2.addWidget(self.ProbationViolationBondButton, 12, 2, 1, 1)
        self.label_2 = QtWidgets.QLabel(self.crimTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_2.setFont(font)
        self.label_2.setAlignment(QtCore.Qt.AlignCenter)
        self.label_2.setObjectName("label_2")
        self.gridLayout_2.addWidget(self.label_2, 10, 0, 1, 3)
        self.pcvh_fcvh_cases_box = QtWidgets.QComboBox(self.crimTab)
        self.pcvh_fcvh_cases_box.setEnabled(False)
        self.pcvh_fcvh_cases_box.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.pcvh_fcvh_cases_box.setObjectName("pcvh_fcvh_cases_box")
        self.gridLayout_2.addWidget(self.pcvh_fcvh_cases_box, 7, 2, 1, 1)
        self.pcvh_fcvh_radioButton = QtWidgets.QRadioButton(self.crimTab)
        self.pcvh_fcvh_radioButton.setObjectName("pcvh_fcvh_radioButton")
        self.buttonGroup_2.addButton(self.pcvh_fcvh_radioButton)
        self.gridLayout_2.addWidget(self.pcvh_fcvh_radioButton, 6, 2, 1, 1)
        self.FailureToAppearButton = QtWidgets.QPushButton(self.crimTab)
        self.FailureToAppearButton.setMinimumSize(QtCore.QSize(120, 30))
        self.FailureToAppearButton.setStyleSheet("background-color: rgb(255, 255, 127);")
        self.FailureToAppearButton.setObjectName("FailureToAppearButton")
        self.gridLayout_2.addWidget(self.FailureToAppearButton, 11, 1, 1, 1)
        self.BondHearingButton = QtWidgets.QPushButton(self.crimTab)
        self.BondHearingButton.setMinimumSize(QtCore.QSize(120, 30))
        self.BondHearingButton.setStyleSheet("background-color: rgb(255, 255, 127);")
        self.BondHearingButton.setObjectName("BondHearingButton")
        self.gridLayout_2.addWidget(self.BondHearingButton, 13, 2, 1, 1)
        self.line_2 = QtWidgets.QFrame(self.crimTab)
        self.line_2.setFrameShadow(QtWidgets.QFrame.Raised)
        self.line_2.setLineWidth(2)
        self.line_2.setFrameShape(QtWidgets.QFrame.HLine)
        self.line_2.setObjectName("line_2")
        self.gridLayout_2.addWidget(self.line_2, 9, 0, 1, 3)
        self.DiversionButton = QtWidgets.QPushButton(self.crimTab)
        self.DiversionButton.setMinimumSize(QtCore.QSize(120, 30))
        self.DiversionButton.setStyleSheet("background-color: rgb(85, 170, 127);")
        self.DiversionButton.setObjectName("DiversionButton")
        self.gridLayout_2.addWidget(self.DiversionButton, 13, 0, 1, 1)
        self.reload_cases_Button = QtWidgets.QPushButton(self.crimTab)
        self.reload_cases_Button.setMinimumSize(QtCore.QSize(120, 30))
        self.reload_cases_Button.setStyleSheet("background-color: rgb(160, 160, 160);")
        self.reload_cases_Button.setObjectName("reload_cases_Button")
        self.gridLayout_2.addWidget(self.reload_cases_Button, 8, 2, 1, 1)
        self.PleaOnlyButton = QtWidgets.QPushButton(self.crimTab)
        self.PleaOnlyButton.setStyleSheet("background-color: rgb(85, 170, 127);")
        self.PleaOnlyButton.setObjectName("PleaOnlyButton")
        self.gridLayout_2.addWidget(self.PleaOnlyButton, 13, 1, 1, 1)
        self.gridLayout_2.setRowStretch(0, 1)
        self.gridLayout_2.setRowStretch(1, 1)
        self.gridLayout_2.setRowStretch(2, 1)
        self.gridLayout_2.setRowStretch(3, 2)
        self.gridLayout_2.setRowStretch(4, 2)
        self.gridLayout_2.setRowStretch(5, 2)
        self.gridLayout_2.setRowStretch(6, 2)
        self.gridLayout_2.setRowStretch(7, 1)
        self.gridLayout_2.setRowStretch(8, 2)
        self.gridLayout_2.setRowStretch(9, 2)
        self.gridLayout_2.setRowStretch(10, 2)
        self.gridLayout_2.setRowStretch(11, 2)
        self.tabWidget.addTab(self.crimTab, "")
        self.schedulingTab = QtWidgets.QWidget()
        self.schedulingTab.setObjectName("schedulingTab")
        self.gridLayout = QtWidgets.QGridLayout(self.schedulingTab)
        self.gridLayout.setObjectName("gridLayout")
        self.label_15 = QtWidgets.QLabel(self.schedulingTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setWeight(75)
        self.label_15.setFont(font)
        self.label_15.setObjectName("label_15")
        self.gridLayout.addWidget(self.label_15, 0, 0, 1, 1)
        self.ac_dattilo_radioButton = QtWidgets.QRadioButton(self.schedulingTab)
        self.ac_dattilo_radioButton.setObjectName("ac_dattilo_radioButton")
        self.gridLayout.addWidget(self.ac_dattilo_radioButton, 0, 1, 1, 1)
        self.ac_patterson_radioButton = QtWidgets.QRadioButton(self.schedulingTab)
        self.ac_patterson_radioButton.setObjectName("ac_patterson_radioButton")
        self.gridLayout.addWidget(self.ac_patterson_radioButton, 0, 2, 1, 1)
        self.radioButton_13 = QtWidgets.QRadioButton(self.schedulingTab)
        self.radioButton_13.setObjectName("radioButton_13")
        self.gridLayout.addWidget(self.radioButton_13, 1, 0, 1, 1)
        self.label_8 = QtWidgets.QLabel(self.schedulingTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_8.setFont(font)
        self.label_8.setAlignment(QtCore.Qt.AlignCenter)
        self.label_8.setObjectName("label_8")
        self.gridLayout.addWidget(self.label_8, 2, 0, 1, 3)
        self.arraignments_radioButton_schdtab = QtWidgets.QRadioButton(self.schedulingTab)
        self.arraignments_radioButton_schdtab.setObjectName("arraignments_radioButton_schdtab")
        self.gridLayout.addWidget(self.arraignments_radioButton_schdtab, 3, 0, 1, 1)
        self.slated_radioButton_schdtab = QtWidgets.QRadioButton(self.schedulingTab)
        self.slated_radioButton_schdtab.setObjectName("slated_radioButton_schdtab")
        self.gridLayout.addWidget(self.slated_radioButton_schdtab, 3, 1, 1, 1)
        self.final_pretrial_radioButton_schdtab = QtWidgets.QRadioButton(self.schedulingTab)
        self.final_pretrial_radioButton_schdtab.setObjectName("final_pretrial_radioButton_schdtab")
        self.gridLayout.addWidget(self.final_pretrial_radioButton_schdtab, 3, 2, 1, 1)
        self.arraignments_cases_box_schdtab = QtWidgets.QComboBox(self.schedulingTab)
        self.arraignments_cases_box_schdtab.setEnabled(False)
        self.arraignments_cases_box_schdtab.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.arraignments_cases_box_schdtab.setEditable(False)
        self.arraignments_cases_box_schdtab.setObjectName("arraignments_cases_box_schdtab")
        self.gridLayout.addWidget(self.arraignments_cases_box_schdtab, 4, 0, 1, 1)
        self.slated_cases_box_schdtab = QtWidgets.QComboBox(self.schedulingTab)
        self.slated_cases_box_schdtab.setEnabled(False)
        self.slated_cases_box_schdtab.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.slated_cases_box_schdtab.setEditable(False)
        self.slated_cases_box_schdtab.setObjectName("slated_cases_box_schdtab")
        self.gridLayout.addWidget(self.slated_cases_box_schdtab, 4, 1, 1, 1)
        self.final_pretrial_cases_box_schdtab = QtWidgets.QComboBox(self.schedulingTab)
        self.final_pretrial_cases_box_schdtab.setEnabled(False)
        self.final_pretrial_cases_box_schdtab.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.final_pretrial_cases_box_schdtab.setEditable(False)
        self.final_pretrial_cases_box_schdtab.setObjectName("final_pretrial_cases_box_schdtab")
        self.gridLayout.addWidget(self.final_pretrial_cases_box_schdtab, 4, 2, 1, 1)
        self.pleas_radioButton_schdtab = QtWidgets.QRadioButton(self.schedulingTab)
        self.pleas_radioButton_schdtab.setObjectName("pleas_radioButton_schdtab")
        self.gridLayout.addWidget(self.pleas_radioButton_schdtab, 5, 0, 1, 1)
        self.trials_to_court_radioButton_schdtab = QtWidgets.QRadioButton(self.schedulingTab)
        self.trials_to_court_radioButton_schdtab.setObjectName("trials_to_court_radioButton_schdtab")
        self.gridLayout.addWidget(self.trials_to_court_radioButton_schdtab, 5, 1, 1, 1)
        self.pcvh_fcvh_radioButton_schdtab = QtWidgets.QRadioButton(self.schedulingTab)
        self.pcvh_fcvh_radioButton_schdtab.setObjectName("pcvh_fcvh_radioButton_schdtab")
        self.gridLayout.addWidget(self.pcvh_fcvh_radioButton_schdtab, 5, 2, 1, 1)
        self.pleas_cases_box_schdtab = QtWidgets.QComboBox(self.schedulingTab)
        self.pleas_cases_box_schdtab.setEnabled(False)
        self.pleas_cases_box_schdtab.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.pleas_cases_box_schdtab.setObjectName("pleas_cases_box_schdtab")
        self.gridLayout.addWidget(self.pleas_cases_box_schdtab, 6, 0, 1, 1)
        self.trials_to_court_cases_box_schdtab = QtWidgets.QComboBox(self.schedulingTab)
        self.trials_to_court_cases_box_schdtab.setEnabled(False)
        self.trials_to_court_cases_box_schdtab.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.trials_to_court_cases_box_schdtab.setObjectName("trials_to_court_cases_box_schdtab")
        self.gridLayout.addWidget(self.trials_to_court_cases_box_schdtab, 6, 1, 1, 1)
        self.pcvh_fcvh_cases_box_schdtab = QtWidgets.QComboBox(self.schedulingTab)
        self.pcvh_fcvh_cases_box_schdtab.setEnabled(False)
        self.pcvh_fcvh_cases_box_schdtab.setStyleSheet("background-color: rgb(255, 255, 255);")
        self.pcvh_fcvh_cases_box_schdtab.setObjectName("pcvh_fcvh_cases_box_schdtab")
        self.gridLayout.addWidget(self.pcvh_fcvh_cases_box_schdtab, 6, 2, 1, 1)
        self.label_16 = QtWidgets.QLabel(self.schedulingTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_16.setFont(font)
        self.label_16.setAlignment(QtCore.Qt.AlignCenter)
        self.label_16.setObjectName("label_16")
        self.gridLayout.addWidget(self.label_16, 7, 1, 1, 1)
        self.pushButton_12 = QtWidgets.QPushButton(self.schedulingTab)
        self.pushButton_12.setObjectName("pushButton_12")
        self.gridLayout.addWidget(self.pushButton_12, 8, 0, 1, 1)
        self.pushButton_13 = QtWidgets.QPushButton(self.schedulingTab)
        self.pushButton_13.setObjectName("pushButton_13")
        self.gridLayout.addWidget(self.pushButton_13, 8, 1, 1, 1)
        self.schedulingEntryButton = QtWidgets.QPushButton(self.schedulingTab)
        self.schedulingEntryButton.setStyleSheet("background-color: rgb(85, 170, 255);")
        self.schedulingEntryButton.setObjectName("schedulingEntryButton")
        self.gridLayout.addWidget(self.schedulingEntryButton, 8, 2, 1, 1)
        self.tabWidget.addTab(self.schedulingTab, "")
        self.civilTab = QtWidgets.QWidget()
        self.civilTab.setObjectName("civilTab")
        self.gridLayout_4 = QtWidgets.QGridLayout(self.civilTab)
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.label_4 = QtWidgets.QLabel(self.civilTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_4.setFont(font)
        self.label_4.setObjectName("label_4")
        self.gridLayout_4.addWidget(self.label_4, 0, 0, 1, 1)
        self.FinalJudgmentEntryButton_11 = QtWidgets.QPushButton(self.civilTab)
        self.FinalJudgmentEntryButton_11.setMinimumSize(QtCore.QSize(120, 30))
        self.FinalJudgmentEntryButton_11.setObjectName("FinalJudgmentEntryButton_11")
        self.gridLayout_4.addWidget(self.FinalJudgmentEntryButton_11, 1, 0, 1, 1)
        self.FinalJudgmentEntryButton_12 = QtWidgets.QPushButton(self.civilTab)
        self.FinalJudgmentEntryButton_12.setMinimumSize(QtCore.QSize(120, 30))
        self.FinalJudgmentEntryButton_12.setObjectName("FinalJudgmentEntryButton_12")
        self.gridLayout_4.addWidget(self.FinalJudgmentEntryButton_12, 1, 1, 1, 1)
        self.MinorMisdemeanorTrafficButton_3 = QtWidgets.QPushButton(self.civilTab)
        self.MinorMisdemeanorTrafficButton_3.setMinimumSize(QtCore.QSize(120, 30))
        self.MinorMisdemeanorTrafficButton_3.setObjectName("MinorMisdemeanorTrafficButton_3")
        self.gridLayout_4.addWidget(self.MinorMisdemeanorTrafficButton_3, 1, 2, 1, 1)
        self.FinalJudgmentEntryButton_13 = QtWidgets.QPushButton(self.civilTab)
        self.FinalJudgmentEntryButton_13.setMinimumSize(QtCore.QSize(120, 30))
        self.FinalJudgmentEntryButton_13.setObjectName("FinalJudgmentEntryButton_13")
        self.gridLayout_4.addWidget(self.FinalJudgmentEntryButton_13, 2, 0, 1, 1)
        self.FinalJudgmentEntryButton_14 = QtWidgets.QPushButton(self.civilTab)
        self.FinalJudgmentEntryButton_14.setMinimumSize(QtCore.QSize(120, 30))
        self.FinalJudgmentEntryButton_14.setObjectName("FinalJudgmentEntryButton_14")
        self.gridLayout_4.addWidget(self.FinalJudgmentEntryButton_14, 2, 1, 1, 1)
        self.FinalJudgmentEntryButton_15 = QtWidgets.QPushButton(self.civilTab)
        self.FinalJudgmentEntryButton_15.setMinimumSize(QtCore.QSize(120, 30))
        self.FinalJudgmentEntryButton_15.setObjectName("FinalJudgmentEntryButton_15")
        self.gridLayout_4.addWidget(self.FinalJudgmentEntryButton_15, 2, 2, 1, 1)
        self.tabWidget.addTab(self.civilTab, "")
        self.adminTab = QtWidgets.QWidget()
        self.adminTab.setObjectName("adminTab")
        self.gridLayout_3 = QtWidgets.QGridLayout(self.adminTab)
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.label_6 = QtWidgets.QLabel(self.adminTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setWeight(75)
        self.label_6.setFont(font)
        self.label_6.setObjectName("label_6")
        self.gridLayout_3.addWidget(self.label_6, 0, 0, 1, 1)
        self.radioButton_4 = QtWidgets.QRadioButton(self.adminTab)
        self.radioButton_4.setObjectName("radioButton_4")
        self.gridLayout_3.addWidget(self.radioButton_4, 0, 1, 1, 2)
        self.radioButton = QtWidgets.QRadioButton(self.adminTab)
        self.radioButton.setObjectName("radioButton")
        self.gridLayout_3.addWidget(self.radioButton, 0, 3, 1, 1)
        self.radioButton_3 = QtWidgets.QRadioButton(self.adminTab)
        self.radioButton_3.setObjectName("radioButton_3")
        self.gridLayout_3.addWidget(self.radioButton_3, 1, 1, 1, 2)
        self.radioButton_2 = QtWidgets.QRadioButton(self.adminTab)
        self.radioButton_2.setObjectName("radioButton_2")
        self.gridLayout_3.addWidget(self.radioButton_2, 1, 3, 1, 1)
        self.label_7 = QtWidgets.QLabel(self.adminTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_7.setFont(font)
        self.label_7.setObjectName("label_7")
        self.gridLayout_3.addWidget(self.label_7, 3, 0, 1, 2)
        self.pushButton_4 = QtWidgets.QPushButton(self.adminTab)
        self.pushButton_4.setObjectName("pushButton_4")
        self.gridLayout_3.addWidget(self.pushButton_4, 4, 0, 1, 2)
        self.pushButton_5 = QtWidgets.QPushButton(self.adminTab)
        self.pushButton_5.setObjectName("pushButton_5")
        self.gridLayout_3.addWidget(self.pushButton_5, 4, 2, 1, 1)
        self.pushButton_6 = QtWidgets.QPushButton(self.adminTab)
        self.pushButton_6.setObjectName("pushButton_6")
        self.gridLayout_3.addWidget(self.pushButton_6, 4, 3, 1, 1)
        self.label_5 = QtWidgets.QLabel(self.adminTab)
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
        font.setPointSize(11)
        font.setBold(True)
        font.setItalic(False)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_5.setFont(font)
        self.label_5.setObjectName("label_5")
        self.gridLayout_3.addWidget(self.label_5, 5, 0, 1, 2)
        self.JurorPaymentButton = QtWidgets.QPushButton(self.adminTab)
        self.JurorPaymentButton.setObjectName("JurorPaymentButton")
        self.gridLayout_3.addWidget(self.JurorPaymentButton, 6, 0, 1, 2)
        self.pushButton_2 = QtWidgets.QPushButton(self.adminTab)
        self.pushButton_2.setObjectName("pushButton_2")
        self.gridLayout_3.addWidget(self.pushButton_2, 6, 2, 1, 1)
        self.pushButton_3 = QtWidgets.QPushButton(self.adminTab)
        self.pushButton_3.setObjectName("pushButton_3")
        self.gridLayout_3.addWidget(self.pushButton_3, 6, 3, 1, 1)
        self.radioButton_5 = QtWidgets.QRadioButton(self.adminTab)
        self.radioButton_5.setObjectName("radioButton_5")
        self.gridLayout_3.addWidget(self.radioButton_5, 1, 0, 1, 1)
        self.tabWidget.addTab(self.adminTab, "")
        self.gridLayout_5.addWidget(self.tabWidget, 0, 0, 1, 1)
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 979, 27))
        self.menubar.setObjectName("menubar")
        self.menuFile = QtWidgets.QMenu(self.menubar)
        self.menuFile.setObjectName("menuFile")
        self.menuSettings = QtWidgets.QMenu(self.menubar)
        self.menuSettings.setObjectName("menuSettings")
        self.menuView = QtWidgets.QMenu(self.menubar)
        self.menuView.setObjectName("menuView")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)
        self.actionOpen = QtWidgets.QAction(MainWindow)
        self.actionOpen.setObjectName("actionOpen")
        self.actionOptions = QtWidgets.QAction(MainWindow)
        self.actionOptions.setObjectName("actionOptions")
        self.actionExit = QtWidgets.QAction(MainWindow)
        self.actionExit.setObjectName("actionExit")
        self.menu_file_exit = QtWidgets.QAction(MainWindow)
        self.menu_file_exit.setObjectName("menu_file_exit")
        self.actionGeneral_Motion_Entry = QtWidgets.QAction(MainWindow)
        self.actionGeneral_Motion_Entry.setObjectName("actionGeneral_Motion_Entry")
        self.actionFail_to_Appear_Minor_Traffic_Offense = QtWidgets.QAction(MainWindow)
        self.actionFail_to_Appear_Minor_Traffic_Offense.setObjectName("actionFail_to_Appear_Minor_Traffic_Offense")
        self.actionOmnibus_Motion_Form = QtWidgets.QAction(MainWindow)
        self.actionOmnibus_Motion_Form.setObjectName("actionOmnibus_Motion_Form")
        self.actionJury_Instructions = QtWidgets.QAction(MainWindow)
        self.actionJury_Instructions.setObjectName("actionJury_Instructions")
        self.actioncontrollers = QtWidgets.QAction(MainWindow)
        self.actioncontrollers.setObjectName("actioncontrollers")
        self.actionTemplates = QtWidgets.QAction(MainWindow)
        self.actionTemplates.setObjectName("actionTemplates")
        self.menuFile.addAction(self.actionOpen)
        self.menuFile.addAction(self.actionOptions)
        self.menuFile.addAction(self.actionExit)
        self.menuFile.addSeparator()
        self.menuFile.addAction(self.menu_file_exit)
        self.menuSettings.addAction(self.actioncontrollers)
        self.menubar.addAction(self.menuFile.menuAction())
        self.menubar.addAction(self.menuSettings.menuAction())
        self.menubar.addAction(self.menuView.menuAction())

        self.retranslateUi(MainWindow)
        self.tabWidget.setCurrentIndex(1)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)
        MainWindow.setTabOrder(self.hemmeter_radioButton, self.rohrer_radioButton)
        MainWindow.setTabOrder(self.rohrer_radioButton, self.bunner_radioButton)
        MainWindow.setTabOrder(self.bunner_radioButton, self.pelanda_radioButton)
        MainWindow.setTabOrder(self.pelanda_radioButton, self.kudela_radioButton)
        MainWindow.setTabOrder(self.kudela_radioButton, self.arraignments_radioButton)
        MainWindow.setTabOrder(self.arraignments_radioButton, self.slated_radioButton)
        MainWindow.setTabOrder(self.slated_radioButton, self.final_pretrial_radioButton)
        MainWindow.setTabOrder(self.final_pretrial_radioButton, self.pleas_radioButton)
        MainWindow.setTabOrder(self.pleas_radioButton, self.trials_to_court_radioButton)
        MainWindow.setTabOrder(self.trials_to_court_radioButton, self.pcvh_fcvh_radioButton)
        MainWindow.setTabOrder(self.pcvh_fcvh_radioButton, self.reload_cases_Button)
        MainWindow.setTabOrder(self.reload_cases_Button, self.FineOnlyPleaButton)
        MainWindow.setTabOrder(self.FineOnlyPleaButton, self.FailureToAppearButton)
        MainWindow.setTabOrder(self.FailureToAppearButton, self.NotGuiltyBondButton)
        MainWindow.setTabOrder(self.NotGuiltyBondButton, self.JailCCPleaButton)
        MainWindow.setTabOrder(self.JailCCPleaButton, self.ProbationViolationBondButton)
        MainWindow.setTabOrder(self.ProbationViolationBondButton, self.DiversionButton)
        MainWindow.setTabOrder(self.DiversionButton, self.BondHearingButton)
        MainWindow.setTabOrder(self.BondHearingButton, self.FinalJudgmentEntryButton_11)
        MainWindow.setTabOrder(self.FinalJudgmentEntryButton_11, self.arraignments_cases_box)
        MainWindow.setTabOrder(self.arraignments_cases_box, self.slated_cases_box)
        MainWindow.setTabOrder(self.slated_cases_box, self.final_pretrial_cases_box)
        MainWindow.setTabOrder(self.final_pretrial_cases_box, self.pleas_cases_box)
        MainWindow.setTabOrder(self.pleas_cases_box, self.trials_to_court_cases_box)
        MainWindow.setTabOrder(self.trials_to_court_cases_box, self.pcvh_fcvh_cases_box)
        MainWindow.setTabOrder(self.pcvh_fcvh_cases_box, self.FinalJudgmentEntryButton_12)
        MainWindow.setTabOrder(self.FinalJudgmentEntryButton_12, self.MinorMisdemeanorTrafficButton_3)
        MainWindow.setTabOrder(self.MinorMisdemeanorTrafficButton_3, self.tabWidget)
        MainWindow.setTabOrder(self.tabWidget, self.FinalJudgmentEntryButton_13)
        MainWindow.setTabOrder(self.FinalJudgmentEntryButton_13, self.FinalJudgmentEntryButton_14)
        MainWindow.setTabOrder(self.FinalJudgmentEntryButton_14, self.FinalJudgmentEntryButton_15)
        MainWindow.setTabOrder(self.FinalJudgmentEntryButton_15, self.radioButton_4)
        MainWindow.setTabOrder(self.radioButton_4, self.radioButton)
        MainWindow.setTabOrder(self.radioButton, self.radioButton_3)
        MainWindow.setTabOrder(self.radioButton_3, self.radioButton_2)
        MainWindow.setTabOrder(self.radioButton_2, self.pushButton_4)
        MainWindow.setTabOrder(self.pushButton_4, self.pushButton_5)
        MainWindow.setTabOrder(self.pushButton_5, self.pushButton_6)
        MainWindow.setTabOrder(self.pushButton_6, self.JurorPaymentButton)
        MainWindow.setTabOrder(self.JurorPaymentButton, self.pushButton_2)
        MainWindow.setTabOrder(self.pushButton_2, self.pushButton_3)
        MainWindow.setTabOrder(self.pushButton_3, self.radioButton_5)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MuniEntry - ver 0.19.0"))
        self.hemmeter_radioButton.setText(_translate("MainWindow", "Judge Hemmeter"))
        self.rohrer_radioButton.setText(_translate("MainWindow", "Judge Rohrer"))
        self.pleas_radioButton.setText(_translate("MainWindow", "Pleas and Motion/Oral Hearings"))
        self.trials_to_court_radioButton.setText(_translate("MainWindow", "Trials to Court and Jury Trials"))
        self.kudela_radioButton.setText(_translate("MainWindow", "Magistrate Kudela"))
        self.pelanda_radioButton.setText(_translate("MainWindow", "Magistrate Pelanda"))
        self.label_3.setText(_translate("MainWindow", "DAILY CASE LISTS"))
        self.arraignments_radioButton.setText(_translate("MainWindow", "Arraignments"))
        self.NotGuiltyBondButton.setText(_translate("MainWindow", "Not Guilty / Bond"))
        self.slated_radioButton.setText(_translate("MainWindow", "Slated                                  "))
        self.label.setText(_translate("MainWindow", "JUDICIAL OFFICER:"))
        self.FineOnlyPleaButton.setText(_translate("MainWindow", "Fine Only Plea"))
        self.JailCCPleaButton.setText(_translate("MainWindow", "Jail and Comm. Control Plea"))
        self.bunner_radioButton.setText(_translate("MainWindow", "Magistrate Bunner"))
        self.final_pretrial_radioButton.setText(_translate("MainWindow", "Final Pre-trials"))
        self.ProbationViolationBondButton.setText(_translate("MainWindow", "Prelim. Probation Violation / Bond"))
        self.label_2.setText(_translate("MainWindow", "CRIMINAL TRAFFIC ENTRIES"))
        self.pcvh_fcvh_radioButton.setText(_translate("MainWindow", "PCVH/FCVH"))
        self.FailureToAppearButton.setText(_translate("MainWindow", "Failure to Appear"))
        self.BondHearingButton.setText(_translate("MainWindow", "Bond Modification/Revocation"))
        self.DiversionButton.setText(_translate("MainWindow", "Diversion Plea"))
        self.reload_cases_Button.setText(_translate("MainWindow", "Reload Cases"))
        self.PleaOnlyButton.setText(_translate("MainWindow", "Plea Future Sentence Date"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.crimTab), _translate("MainWindow", "Crim/Traffic"))
        self.label_15.setText(_translate("MainWindow", "Staff Member:"))
        self.ac_dattilo_radioButton.setText(_translate("MainWindow", "Assignment Commissioner Dattilo"))
        self.ac_patterson_radioButton.setText(_translate("MainWindow", "Assignment Commissioner Patterson"))
        self.radioButton_13.setText(_translate("MainWindow", "No Signature"))
        self.label_8.setText(_translate("MainWindow", "DAILY CASE LISTS"))
        self.arraignments_radioButton_schdtab.setText(_translate("MainWindow", "Arraignments"))
        self.slated_radioButton_schdtab.setText(_translate("MainWindow", "Slated                                  "))
        self.final_pretrial_radioButton_schdtab.setText(_translate("MainWindow", "Final Pre-trials"))
        self.pleas_radioButton_schdtab.setText(_translate("MainWindow", "Pleas and Motion/Oral Hearings"))
        self.trials_to_court_radioButton_schdtab.setText(_translate("MainWindow", "Trials to Court and Jury Trials"))
        self.pcvh_fcvh_radioButton_schdtab.setText(_translate("MainWindow", "PCVH/FCVH"))
        self.label_16.setText(_translate("MainWindow", "SCHEDULING ENTRIES"))
        self.pushButton_12.setText(_translate("MainWindow", "PushButton"))
        self.pushButton_13.setText(_translate("MainWindow", "PushButton"))
        self.schedulingEntryButton.setText(_translate("MainWindow", "Scheduling Entry"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.schedulingTab), _translate("MainWindow", "Scheduling"))
        self.label_4.setText(_translate("MainWindow", "Civil Entries"))
        self.FinalJudgmentEntryButton_11.setText(_translate("MainWindow", "Entry"))
        self.FinalJudgmentEntryButton_12.setText(_translate("MainWindow", "Entry"))
        self.MinorMisdemeanorTrafficButton_3.setText(_translate("MainWindow", "Entry"))
        self.FinalJudgmentEntryButton_13.setText(_translate("MainWindow", "Entry"))
        self.FinalJudgmentEntryButton_14.setText(_translate("MainWindow", "Entry"))
        self.FinalJudgmentEntryButton_15.setText(_translate("MainWindow", "Entry"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.civilTab), _translate("MainWindow", "Civil"))
        self.label_6.setText(_translate("MainWindow", "Staff Member:"))
        self.radioButton_4.setText(_translate("MainWindow", "Court Administrator Kudela"))
        self.radioButton.setText(_translate("MainWindow", "Assignment Commissioner Dattilo"))
        self.radioButton_3.setText(_translate("MainWindow", "Assignment Commissioner Travis"))
        self.radioButton_2.setText(_translate("MainWindow", "Jury Commissioner Travis"))
        self.label_7.setText(_translate("MainWindow", "Letters"))
        self.pushButton_4.setText(_translate("MainWindow", "Judge No Review"))
        self.pushButton_5.setText(_translate("MainWindow", "PushButton"))
        self.pushButton_6.setText(_translate("MainWindow", "PushButton"))
        self.label_5.setText(_translate("MainWindow", "Administrative Entries"))
        self.JurorPaymentButton.setText(_translate("MainWindow", "Juror Payment"))
        self.pushButton_2.setText(_translate("MainWindow", "PushButton"))
        self.pushButton_3.setText(_translate("MainWindow", "PushButton"))
        self.radioButton_5.setText(_translate("MainWindow", "No Signature"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.adminTab), _translate("MainWindow", "Admin"))
        self.menuFile.setTitle(_translate("MainWindow", "File"))
        self.menuSettings.setTitle(_translate("MainWindow", "Settings"))
        self.menuView.setTitle(_translate("MainWindow", "View"))
        self.actionOpen.setText(_translate("MainWindow", "Open"))
        self.actionOptions.setText(_translate("MainWindow", "Options"))
        self.actionExit.setText(_translate("MainWindow", "Templates"))
        self.menu_file_exit.setText(_translate("MainWindow", "Exit"))
        self.actionGeneral_Motion_Entry.setText(_translate("MainWindow", "General Motion Entry"))
        self.actionFail_to_Appear_Minor_Traffic_Offense.setText(_translate("MainWindow", "Fail to Appear - Minor Traffic Offense"))
        self.actionOmnibus_Motion_Form.setText(_translate("MainWindow", "Omnibus Motion Form"))
        self.actionJury_Instructions.setText(_translate("MainWindow", "Jury Instructions"))
        self.actioncontrollers.setText(_translate("MainWindow", "controllers"))
        self.actionTemplates.setText(_translate("MainWindow", "Templates"))
