...
civil_dialog_buttons = [
    ('CivFreeformEntryButton', 'Civil Freeform Entry Case Information'),
]


@pytest.mark.parametrize(TEST_LIST, civil_dialog_buttons)
...
def test_dialogs_open_nocase_from_casesearch(main_window, dialog_button, dialog_title):
    """Tests all Civil dialog buttons open from the case search when no case is selected."""
    mouse_click(main_window.judge_1_radio_btn)
...
#     main_window.cases_tab_widget.setCurrentWidget(main_window.case_search_tab)
